...
const _allOptions = ["JavaScript", "HTML", "CSS", "React", "Node.js", "Python"];

const SearchableSelect = ({ selectedOptions = [], onOptionSelect }: any) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [isOptionsVisible, setIsOptionsVisible] = useState(false);
  const [filteredOptions, setFilteredOptions] = useState<Array<string>>(_allOptions);

  useEffect(() => {
    // Filter out the selected options from the available options
    const availableOptions = _allOptions.filter(option => !selectedOptions.includes(option));
  ...
  const filterOptions = (value: any) => {
    const filtered = _allOptions.filter((option: any) =>
  ...
  return (
    <div className={`${styles.searchableSelect} relative h-10 w-full min-w-[300px] my-[18px]`}>
      <input
        type="text"
        value={searchTerm}
        onFocus={showOptions}
